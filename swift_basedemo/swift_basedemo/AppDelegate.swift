...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        //设置状态栏颜色
        let statusBackgoundView = UIView();
        statusBackgoundView.backgroundColor = UIColor.rgba(red: 0, green: 0, blue: 0, alpha:0.2);
        window?.addSubview(statusBackgoundView);
        window?.addConstraintsWithVisual(format: "H:|[v0]|", views: statusBackgoundView);
        let rect = UIApplication.shared.statusBarFrame;
        window?.addConstraintsWithVisual(format: "V:|[v0(\(rect.size.height))]", views: statusBackgoundView);
        return true
    }
